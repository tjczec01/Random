...
__all__ = ["RadauD", "solve_ivpd", "dm", "Sf", "flatten", "solve_collocation_system",  "DCA", "DCS", "DCM", "DCD", "predict_factor", "RadauDenseOutput", "ComplexDecimal", "normd", "CP", "Tile", "Tfunc"]
...
def solve_collocation_system(fun, t, y, h, Z0, scale, tol,
                             LU_real, LU_complex, solve_lu, solve_luc, prec):
    ...
    """
    TD = df.DF(T, prec).decfunc()
    TID = df.DF(TI, prec).decfunc()
    E = [dm(ei, prec)/dm(3.0, prec) for ei in Eb]
    Ef = df.DF(E, prec).decfuncl(E)
    n = y.shape[0]
    M_real = MU_REAL / h
    M_complex = MU_COMPLEX / float(h) #DCD(MU_COMPLEX, h, prec) 
    ch = np.array([dm(h*i, prec) for i in C])
    ZP = np.zeros((S, n))
    Z = Z0
    W = np.array(TID).dot(df.DF(Z0, prec).decfunc())
    for i in range(S):
            ZD = [dm(i, prec) for i in Z[i]]
            YD = [dm(i, prec) for i in y]
            # print(fun(dm(t, prec) + ch[i], y + Z[i]))
            ZP[i] = fun(dm(t, prec) + ch[i], np.array(YD) + np.array(ZD))
            # for j in range(n):
            #        try:
            #            ZP[i][j] = fun(dm(t + dm(h, prec)*dm(C[i], prec), prec), dm(y, prec) + (dm(Z0[i][j], prec))).tolist()
            #        except:
            #            ZP[i][j] = [dm(fun(dm(t + dm(h, prec)*dm(C[i], prec), prec), dm(y[0], prec) + (dm(Z0[i][j], prec))).tolist()[0], prec)]
    Z0P = np.array(flatten(ZP))
    Zd = df.DF(Z0, prec).decfunc()
    # try:
    ...
    for k in range(NEWTON_MAXITER):
        for i in range(S):
             ...
             ZD = [dm(i, prec) for i in Z[i]] 
             YD = [dm(i, prec) for i in y]
             F[i] = fun(dm(t, prec) + ch[i], np.array(YD) + np.array(ZD))
                   # try:
                   #     F[i] = fun(dm(t, prec) + ch[i], y + Z[i])
                   # except:
                   #     try:
        ...
        # print(dm(M_real, prec) * W[0])
        MR = [dm(M_real, prec) *  dm(W[0][i], prec) for i in range(len(W[0]))]
        f_real = [dm(F.T.dot(TI_REAL)[i], prec) - MR[i] for i in range(len(W[0]))]
        # print(f_real.tolist())
        # try:
        #     try:
        #         f_real = [dm(F.T.dot(TI_REAL), prec) - dm(M_real[0], prec) * dm(W[0], prec)]
        #     except:
        #         try:
        ...
        MC = [M_complex *  complex(W[1][i],  W[2][i]) for i in range(len(W[1]))]
        f_complex = F.T.dot(TI_COMPLEX) -  np.array(MC) #np.array(MC)
        # print(f_complex.tolist())
        # print(f_real)
        dW_real = solve_lu(LU_real[0], LU_real[1], f_real)
        # print(dW_real)
        dW_complex = solve_luc(LU_complex[0], LU_complex[1], f_complex.tolist())
        dW[0] = [dm(i.real, prec) for i in dW_real]
        dW[1] = [dm(i.real, prec) for i in dW_complex]
        dW[2] = [dm(i.imag, prec) for i in dW_complex]
        dW2[0] = dW_real[0].real
        dW2[1] = dW_complex[0].real
        dW2[2] = dW_complex[0].imag
        dW_norm = normd(np.array(dW, dtype=float)/scale, prec)
...
class RadauD(OdeSolver):
    ...
    def __init__(self, fun, t0, y0, t_bound, prec,  max_step=np.inf,
        ...
        Ef = df.DF(E, self.prec).decfuncl(E)
        TD = df.DF(T, self.prec).decfunc()
        TID = df.DF(TI, self.prec).decfunc()
        PD = df.DF(P, self.prec).decfunc()
        ...
        if issparse(self.J):
            ...
            def solve_luc(L, U, b):
                try:
                      ...
                      B = [complex(i.real, i.imag) for i in b]
                AA = df.DF([L, U], self.prec).lu_solvec(L, U, B)
                return AA
    ...
    def _step_impl(self):
        ...
        while not step_accepted:
            ...
            if self.sol is None:
                ...
                CH = np.array([dm(t + h*i, self.prec) for i in C])
            ...
            scale = atol + np.array(np.abs(y), dtype=float) * rtol
            ...
            while not converged:
                if LU_real is None or LU_complex is None:
                    ...
                    # print(J)
                    vr = dm(MU_REAL, self.prec) / dm(h, self.prec)
                    vc = MU_COMPLEX / complex(h)
                    IR = df.DF([0.0], self.prec).idenv(len(J), vr)
                    RF = df.DF([0.0], self.prec).matrixsub(IR, J)
                    # print(RF)
                ...
                converged, n_iter, Z, rate = solve_collocation_system(
                    ...
                    LU_real, LU_complex, self.solve_lu, self.solve_luc, self.prec)
            ...
            if not converged:
                ...
                continue
            try:
                y_new = [dm(y[i], self.prec) for i in range(len(y))] + Z[-1]
            except:
                y_new = y + Z[-1]
            ...
            for i in errorf:
                 iif = errorf.index(i)
                 if float(errorf[iif].imag) == 0.0:
                      error.append(float(errorf[iif].real))
                 else:
                      error.append(float(errorf[iif].real) +  1j*float(errorf[iif].imag))
            # error = np.array([float(mp.re(errorf[i])) +  1j*float(mp.im(errorf[i])) for i in range(len(errorf)) if float(mp.im(errorf[i])) == 0.0])
            # print(errorf)
            # print(error[0], error[1])
            # print(y)
            # print(y_new)
            scale = atol + np.array(np.maximum(np.abs(y), np.abs(y_new)), dtype=float) * rtol
            # print(scale)
            error_norm = norm(np.array(error) / np.array(scale))
            safety = 0.9 * (2 * NEWTON_MAXITER + 1) / (2 * NEWTON_MAXITER + n_iter)

            if rejected and error_norm > 1:
                 errorf = df.DF(LU_real, self.prec).solve(LU_real, self.fun(t, y + error) + ZE)
                 error = []
                 for i in errorf:
                      iif = errorf.index(i)
                      if float(errorf[iif].imag) == 0.0:
    ...
    def _compute_dense_output(self):
        ...
        Q = np.dot(df.DF(self.Z.T, self.prec).decfunc(), df.DF(P, self.prec).decfunc())
...
class RadauDenseOutput(DenseOutput):
    ...
    def _call_impl(self, t):
        ...
        xd = (np.array(t, dtype=float) - self.t_old) / self.h #[dm(xi, self.prec) for xi in x.tolist()]
        if t.ndim == 0:
        ...
        # except:
        #     pl = dm(p[0], self.prec)
        y = np.dot(self.Q, df.DF(p, self.prec).decfunc())
        # yv = y.tolist()[0]
        # print(self.y_old[:])
        yo = []
        for i in range(len(self.y_old)):
             # for j in range(len(self.y_old[:])):
                  yo.append(dm(self.y_old[i, None], self.prec))
        Yold = np.array(yo) 
        if y.ndim == 2:
            y += Yold[:, None]
            # try:
...
def solve_ivpd(fun, t_span, y0, prec, method='RK45',  t_eval=None, dense_output=False,
...
solve_ivpd.__name__ = "solve_ivpd"       
