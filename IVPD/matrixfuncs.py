...
"""
__all__ = ['Gamma', 'LU_decomposition', 'LUdecomp', 'backward_sub', 'comb', 'csc_groups', 'cyclic_form', 'det', 'dot', 'factorial', 'flatten', 'forward_sub', 'iden', 'kcycle', 'linear_solve', 'lu_solve', 'matmul', 'matmullu', 'minlex', 'multiply', 'perm', 'permutations', 'rotate_left', 'rotate_right', 'rowred', 'sgnf', 'solsys', 'sparsity', 'transpose', 'transpositions', 'zero']
...
def csc_array(rows_l, cols_l, data_l):
              rs = max(rows_l)
              cs = max(cols_l)
              Z = zeros_matrix(rs+1, cs+1)
              k = 0
...
CSC = csc_array(row, col, data)

__name__ = "matrixfunctions"